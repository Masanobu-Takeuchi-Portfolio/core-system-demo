            ...
            <form method="get" action="{{ route('admin.fare.search') }}">
                ...
                <div class="flex">
                    ...
                    <select name="month" id="" class="rounded mx-1">
                        @for ($i = 1; $i <= 12; $i++) <option value="{{ $i }}" {{ $calendar->month == $i ? 'selected' : '' }}>
                            {{ $i }}月</option>
                            @endfor
                    </select> --}}
        ...
        <div class="max-w-7xl mx-auto sm:px-6 lg:px-8">
            <div class="bg-white overflow-hidden shadow-sm sm:rounded-lg">
                ...
                <div class="p-2 text-gray-900">
                    <div class="">
                        <div class="flex flex-wrap justify-center items-center  border-t-2 mdmax:border-x-2 md:border-l md:border-r border-slate-600 text-center p-1">
                            <div class="bg-amber-100 mdmax:w-[50%] w-[12%] border-r mdmax:border-b border-slate-500">
                                移動日</div>
                            <div class="bg-amber-100 mdmax:w-[50%] w-[20%] border-r mdmax:border-b border-slate-500">
                                移動手段又は駐車場</div>
                            <div class="bg-amber-100 mdmax:w-[50%] w-[10%] mdmax:border-b mdmax:border-r border-slate-500 md:border-r">
                                乗車駅</div>
                            <div class="bg-amber-100 mdmax:w-[50%] w-[10%] mdmax:border-b border-slate-500 md:border-r">
                                降車駅</div>
                            <div class="bg-amber-100 mdmax:w-[50%] w-[10%]  mdmax:border-b mdmax:border-r border-slate-500 md:border-r">
                                片道・往復</div>
                            <div class="bg-amber-100 mdmax:w-[50%] w-[10%] mdmax:border-b border-slate-500 md:border-r">
                                料金</div>
                            <div class="bg-amber-100 mdmax:w-[100%] w-[28%] mdmax:border-b border-slate-500">
                                目的地・駐車場</div>
                        </div>
                    </div>
                    <form class="mb-8" method="post" action="@isset($fare->id)
                        ...
                        <div class="form-fields-container">
                            @for ($i = 0; $i < $linecum; $i++) 
                            <div class="@if (!isset($fare_data[$i]['date'])) hidden @endif flex flex-wrap justify-center items-center  border-t-2 mdmax:border-x-2 md:border-l md:border-r border-slate-600 text-center p-1">
                                <div class="mdmax:w-[50%] w-[12%] mdmax:border-b border-r border-slate-500">
                                    <input class="datepic" type="text" name="key_date[{{$i}}]" value="{{old('key_date.' . $i , $fare_data[$i]['date'] ?? null)}}" placeholder="日付入力">
                                </div>
                                <div class="mdmax:w-[50%] w-[20%] mdmax:border-b border-slate-500 ">
                                ...
                                <div class="mdmax:w-[100%] w-[28%] mdmax:border-b border-slate-500">
                ...
                {{-- 大阪フォーマットここまで --}}
                <button id="add-field-btn" type="button" class="bg-amber-100 p-1 @if(!$isAdminEditable) hidden @endif">+入力項目を追加する</button>
                ...
                {{-- 追加用フォームここから --}}
                <div class="addForm hidden flex flex-wrap justify-center items-center  border-t-2 mdmax:border-x-2 md:border-l md:border-r border-slate-600 text-center p-1">
                    <div class="mdmax:w-[50%] w-[12%] mdmax:border-b border-r border-slate-500">
                        <input class="datepic" type="text" name="key_date[{{$i}}]" value="{{old('key_date.' . $i , $fare_data[$i]['date'] ?? null)}}" placeholder="日付入力">
                    </div>
                    <div class="mdmax:w-[50%] w-[20%] mdmax:border-b border-slate-500 ">

        ...
        <div class="max-w-7xl mx-auto sm:px-6 lg:px-8">
            <div class="bg-white overflow-hidden shadow-sm sm:rounded-lg">
                <div class="p-6 text-gray-900">
                    <form class="mb-8" method="get" action="{{ route('admin.fare.index') }}">
                        ...
                        <table class="table-auto w-full text-left whitespace-no-wrap">
                            ...
                            <tbody>
                                @foreach ($fares as $fare)
                                    <tr>
                                        <td class="border-t-2 border-gray-200 px-4 py-3">{{ $fare->id }}</td>
                                        <td class="border-t-2 border-gray-200 px-4 py-3">
                                            {{ $fare->name_last }}&nbsp;{{ $fare->name_first }}</td>
                                        <td class="border-t-2 border-gray-200 px-4 py-3">{{ $fare->email }}</td>
                                        <td class="border-t-2 border-gray-200 px-4 py-3">{{ $fare->year }}</td>
                                        <td class="border-t-2 border-gray-200 px-4 py-3">{{ $fare->month }}</td>
                                        <td class="border-t-2 border-gray-200 px-4 py-3">{{ $fare->department }}</td>
                                        <td class="border-t-2 border-gray-200 px-4 py-3">
                                            <a href="{{ route('admin.fare.edit', ['id' => $fare->id, 'year' => $fare->year, 'month' => $fare->month]) }}"
